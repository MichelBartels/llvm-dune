...
" > "$basedir/common/dune"
    ...
    if $support_shared_mode; then
        ...
        cp "src/target/target_ocaml.h" "$basedir/shared"
...
(library
 ...
 (foreign_stubs
  (language c)
  (names ${cfile})
  (extra_deps llvm_ocaml.h target_ocaml.h)
  (flags ($cflags)))
...
" >> "$basedir/shared/dune"
    ...
    if $support_static_mode; then
        ...
        cp "src/target/target_ocaml.h" "$basedir/static"
...
create_dune_file     llvm.transform_utils transforms/utils           llvm_transform_utils transform_utils_ocaml "llvm llvm.target"                "transformutils"
create_dune_file     llvm.passbuilder     transforms/passbuilder     llvm_passbuilder     passbuilder_ocaml     "llvm llvm.target"                "passes"
